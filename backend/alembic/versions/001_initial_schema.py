...
def upgrade() -> None:
    ...
    check_status_enum = postgresql.ENUM(
        'new', 'in_review', 'escalated', 'pending_dual_control',
        'pending_approval', 'approved', 'rejected', 'returned', 'closed',
        name='checkstatus',
        create_type=False
    )
    ...
    account_type_enum = postgresql.ENUM(
        'consumer', 'business', 'commercial', 'non_profit',
        name='accounttype',
        create_type=False
    )
    ...
    decision_type_enum = postgresql.ENUM(
        'review_recommendation', 'approval_decision', 'escalation',
        name='decisiontype',
        create_type=False
    )
    ...
    decision_action_enum = postgresql.ENUM(
        'approve', 'return', 'reject', 'hold', 'escalate', 'needs_more_info',
        name='decisionaction',
        create_type=False
    )
    ...
    queue_type_enum = postgresql.ENUM(
        'standard', 'high_priority', 'escalation', 'special_review',
        name='queuetype',
        create_type=False
    )
    ...
    rule_type_enum = postgresql.ENUM(
        'threshold', 'dual_control', 'escalation', 'routing', 'require_reason',
        name='ruletype',
        create_type=False
    )
    ...
    audit_action_enum = postgresql.ENUM(
        ...
        'mfa_enabled', 'mfa_disabled', 'session_expired', 'token_refreshed',
        # Authorization failures
        'auth_permission_denied', 'auth_role_denied', 'auth_entitlement_denied', 'auth_ip_denied',
        # Check items
        'item_viewed', 'item_assigned', 'item_reassigned', 'item_escalated',
        'item_status_changed', 'item_locked', 'item_unlocked',
        # Decisions - successes
        'decision_made', 'decision_approved', 'decision_rejected',
        # Decisions - failures
        'decision_failed', 'decision_validation_failed', 'decision_entitlement_failed',
        # Decisions - overrides
        'decision_overridden', 'decision_reversed', 'decision_amended',
        # Dual control
        'dual_control_required', 'dual_control_approved', 'dual_control_rejected', 'dual_control_expired',
        # Images
        'image_viewed', 'image_zoomed', 'image_downloaded', 'image_access_denied',
        # Admin
        'user_created', 'user_updated', 'user_deactivated', 'role_assigned', 'role_removed',
        'policy_created', 'policy_updated', 'policy_activated',
        'queue_created', 'queue_updated',
        'entitlement_created', 'entitlement_updated', 'entitlement_revoked',
        # Export
        'audit_packet_generated', 'report_exported', 'report_viewed', 'data_exported',
        # AI inference
        'ai_inference_requested', 'ai_inference_completed', 'ai_inference_failed',
        'ai_assist_viewed', 'ai_assist_feedback',
        'ai_recommendation_accepted', 'ai_recommendation_rejected', 'ai_recommendation_overridden',
        # Security
        'unauthorized_access', 'suspicious_activity', 'rate_limit_exceeded',
        # Fraud Intelligence
        'fraud_event_created', 'fraud_event_submitted', 'fraud_event_withdrawn',
        'fraud_config_updated', 'network_alert_dismissed', 'fraud_match_found', 'fraud_match_reviewed',
        # System events
        'system_config_changed', 'batch_operation_started', 'batch_operation_completed',
    ...
    op.create_table(
        ...
        sa.Column('pending_dual_control_decision_id', sa.String(36), nullable=True),  # FK added later to avoid circular ref
        ...
        # Policy tracking
        sa.Column('policy_version_id', sa.String(36), sa.ForeignKey('policy_versions.id', ondelete='SET NULL')),
        # Multi-tenant
        sa.Column('tenant_id', sa.String(36), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), onupdate=sa.func.now()),
    ...
    op.create_index('ix_check_items_pending_dual_control', 'check_items', ['pending_dual_control_decision_id'])
    ...
    op.create_index('ix_approval_entitlements_active_type', 'approval_entitlements', ['is_active', 'entitlement_type'])
