...
def upgrade() -> None:
    ...
    sharing_level_enum = postgresql.ENUM(
        "0", "1", "2", name="sharing_level", create_type=False  # PRIVATE, AGGREGATE, NETWORK_MATCH
    )
    ...
    match_severity_enum = postgresql.ENUM(
        "low", "medium", "high", name="match_severity", create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "check_item_id",
            sa.String(36),
            sa.ForeignKey("check_items.id", ondelete="SET NULL"),
            index=True,
        ),
        ...
        sa.Column(
            "sharing_level", sa.Integer, nullable=False, default=0
        ),  # 0=Private, 1=Aggregate, 2=Network
    ...
    op.create_table(
        ...
        sa.Column(
            "fraud_event_id",
            sa.String(36),
            sa.ForeignKey("fraud_events.id", ondelete="CASCADE"),
            unique=True,
        ),
        # Sharing level
    ...
    op.create_index(
        "ix_fraud_shared_artifacts_active", "fraud_shared_artifacts", ["is_active", "sharing_level"]
    )
    op.create_index(
        "ix_fraud_shared_artifacts_occurred_month", "fraud_shared_artifacts", ["occurred_month"]
    )
    op.create_index(
        "ix_fraud_shared_artifacts_fraud_type", "fraud_shared_artifacts", ["fraud_type"]
    )
    op.create_index(
        "ix_fraud_shared_artifacts_pepper_version", "fraud_shared_artifacts", ["pepper_version"]
    )
    ...
    op.create_index(
        "ix_network_match_alerts_tenant_check",
        "network_match_alerts",
        ["tenant_id", "check_item_id"],
    )
    op.create_index(
        "ix_network_match_alerts_severity", "network_match_alerts", ["severity", "dismissed_at"]
    )
