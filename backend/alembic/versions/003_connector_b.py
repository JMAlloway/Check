...
down_revision = '002_fraud_intelligence'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('file_format', sa.Enum('csv', 'fixed_width', 'xml', 'json', name='fileformat'), nullable=False, server_default='csv'),
        ...
        sa.Column('delivery_method', sa.Enum('sftp', 'shared_folder', 'message_queue', 'api_callback', name='deliverymethod'), nullable=False, server_default='sftp'),
    ...
    op.create_index('ix_bank_connector_configs_active', 'bank_connector_configs', ['tenant_id', 'is_active'])
    ...
    op.create_table(
        ...
        sa.Column('bank_config_id', sa.String(36), sa.ForeignKey('bank_connector_configs.id'), nullable=False),
        sa.Column('description', sa.Text()),

        # Status tracking
        sa.Column('status', sa.Enum(
            'pending', 'approved', 'generating', 'generated', 'transmitted',
            'acknowledged', 'partially_processed', 'completed', 'failed', 'cancelled',
            name='batchstatus'
        ...
        sa.Column('ack_status', sa.Enum('accepted', 'rejected', 'partially_processed', 'pending', name='acknowledgementstatus')),
    ...
    op.create_table(
        ...
        # Batch linkage
        sa.Column('batch_id', sa.String(36), sa.ForeignKey('commit_batches.id', ondelete='CASCADE'), nullable=False),
        ...
        # Record status
        sa.Column('status', sa.Enum(
            'pending', 'included', 'transmitted', 'accepted', 'rejected',
            'failed', 'retrying', 'manually_resolved',
            name='recordstatus'
        ), nullable=False, server_default='pending'),

        # Decision details
        sa.Column('decision_type', sa.Enum(
            'release', 'hold', 'extend_hold', 'modify_hold', 'return', 'reject', 'escalate',
        ...
        sa.Column('hold_reason_code', sa.Enum(
            'new_account', 'large_deposit', 'redeposit', 'repeated_overdraft',
            'reasonable_doubt', 'emergency_conditions', 'next_day_unavailable', 'other',
            name='holdreasoncode'
        )),
        ...
        # Error handling
        sa.Column('error_category', sa.Enum('validation', 'business_rule', 'system', 'timeout', 'auth', name='errorcategory')),
    ...
    op.create_index('ix_commit_records_batch_sequence', 'commit_records', ['batch_id', 'sequence_number'])
    ...
    op.create_table(
        ...
        # Overall status
        sa.Column('status', sa.Enum('accepted', 'rejected', 'partially_processed', 'pending', name='acknowledgementstatus', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('tenant_id', 'report_date', name='uq_reconciliation_report_tenant_date'),
    )
    op.create_index('ix_reconciliation_reports_tenant_date', 'reconciliation_reports', ['tenant_id', 'report_date'])
