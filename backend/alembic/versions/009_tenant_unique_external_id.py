...
def upgrade() -> None:
    ...
    if dialect == "postgresql":
        ...
        if constraint_name:
            ...
            try:
                op.drop_constraint(
                    "check_items_external_item_id_key", "check_items", type_="unique"
                )
            except Exception:
                # Constraint might not exist or have different name - log and continue
                print(
                    "Warning: Could not drop unique constraint on external_item_id - may already be removed"
                )

        # Create composite unique constraint
        op.create_unique_constraint(
            "uq_check_items_tenant_external_id", "check_items", ["tenant_id", "external_item_id"]
        )
        ...
        with op.batch_alter_table("check_items", recreate="auto") as batch_op:
            ...
            batch_op.create_unique_constraint(
                "uq_check_items_tenant_external_id", ["tenant_id", "external_item_id"]
            )
...
def downgrade() -> None:
    ...
    if dialect == "postgresql":
        ...
        op.create_unique_constraint(
            "check_items_external_item_id_key", "check_items", ["external_item_id"]
        )
        ...
        with op.batch_alter_table("check_items", recreate="auto") as batch_op:
            ...
            batch_op.create_unique_constraint(
                "check_items_external_item_id_key", ["external_item_id"]
            )
