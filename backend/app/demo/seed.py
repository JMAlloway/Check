...
# Bump this whenever the seeder's OUTPUT changes (new image refs, new fields,
# different scenarios, ...). Startup compares it to the stamp stored in
# demo_meta and reseeds automatically when they differ, so persistent dev/demo
# volumes (Codespaces, docker volumes) never keep serving data generated by
# older seed code.
DEMO_SEED_VERSION = 2


async def _stored_seed_version(db: AsyncSession) -> int | None:
    """Read the seed-version stamp, or None if absent/unreadable."""
    from app.models.demo_meta import DemoMeta

    try:
        result = await db.execute(select(DemoMeta.value).where(DemoMeta.key == "seed_version"))
        value = result.scalar_one_or_none()
        return int(value) if value is not None else None
    except Exception:
        # Table missing (pre-versioning volume before create_all ran) etc.
        return None
...
async def _write_seed_version(db: AsyncSession) -> None:
    ...
    existing = await db.get(DemoMeta, "seed_version")
    if existing is None:
        db.add(DemoMeta(key="seed_version", value=str(DEMO_SEED_VERSION)))
    else:
        existing.value = str(DEMO_SEED_VERSION)
    await db.commit()
...
async def seed_demo_data(reset: bool = False, count: int = 250) -> dict:
    ...
    Stale-data handling: demo data carries a seed-version stamp (demo_meta).
    - stamp matches the current DEMO_SEED_VERSION -> data is current, skip
      seeding entirely (the per-step seeders are not safe to re-run over
      existing data, and there is nothing to do anyway);
    - stamp missing or different while demo users exist -> the data was
      generated by older seed code; recreate the schema and reseed so
      persistent volumes (Codespaces, docker volumes) pick up new demo data
      automatically instead of serving stale checks forever.
    """
    if not reset:
        async with AsyncSessionLocal() as db:
            stored = await _stored_seed_version(db)
            has_users = False
            try:
                ...
                pass
            if has_users:
                if stored == DEMO_SEED_VERSION:
                    logger.info(
                        "Demo data is current (seed version %s) - skipping seed",
                        DEMO_SEED_VERSION,
                    )
