...
class AuditPacketGenerator:
    ...
    def _build_check_details(self, item: CheckItem) -> list:
        ...
        data = [
            ...
            [
                "Payee:",
                item.payee_name or "N/A",
                "Check Date:",
                item.check_date.strftime("%Y-%m-%d") if item.check_date else "N/A",
            ],
            [
                "Memo:",
                item.memo or "N/A",
                "Presented:",
                item.presented_date.strftime("%Y-%m-%d %H:%M") if item.presented_date else "N/A",
            ],
        ...
        if item.micr_line:
            ...
            elements.append(
                Paragraph(
                    f"<font face='Courier'>{item.micr_line}</font>", self.styles["FieldValue"]
                )
            )
    ...
    def _build_account_context(self, item: CheckItem) -> list:
        ...
        account_data = [
            [
                ...
                account_type_str,
                "Tenure (Days):",
                str(item.account_tenure_days) if item.account_tenure_days else "N/A",
            ],
            [
                "Current Balance:",
                f"${item.current_balance:,.2f}" if item.current_balance else "N/A",
                "Avg Balance (30d):",
                f"${item.average_balance_30d:,.2f}" if item.average_balance_30d else "N/A",
            ],
        ]
        ...
        behavior_data = [
            [
                "Avg Check (30d):",
                f"${item.avg_check_amount_30d:,.2f}" if item.avg_check_amount_30d else "N/A",
                "Avg Check (90d):",
                f"${item.avg_check_amount_90d:,.2f}" if item.avg_check_amount_90d else "N/A",
            ],
            [
                "Max Check (90d):",
                f"${item.max_check_amount_90d:,.2f}" if item.max_check_amount_90d else "N/A",
                "Std Dev (30d):",
                f"${item.check_std_dev_30d:,.2f}" if item.check_std_dev_30d else "N/A",
            ],
            [
                "Returned (90d):",
                (
                    str(item.returned_item_count_90d)
                    if item.returned_item_count_90d is not None
                    else "N/A"
                ),
                "Exceptions (90d):",
                str(item.exception_count_90d) if item.exception_count_90d is not None else "N/A",
            ],
        ]

        behavior_table = Table(
            behavior_data, colWidths=[1.3 * inch, 2.2 * inch, 1.3 * inch, 2.2 * inch]
        )
    ...
    def _build_images_section(self, item: CheckItem) -> list:
        ...
        if front_image:
            ...
            try:
                ...
                image_ref = (
                    front_image.external_image_id or front_image.storage_path or front_image.id
                )
                elements.append(
                    Paragraph(f"<i>Image Reference: {image_ref}</i>", self.styles["FieldValue"])
                )
            except Exception:
                elements.append(
                    Paragraph("<i>Front image not available</i>", self.styles["FieldValue"])
                )
        else:
        ...
        if back_image:
            ...
            try:
                ...
                elements.append(
                    Paragraph("<i>Back image not available</i>", self.styles["FieldValue"])
                )
        else:
        ...
        for decision in decisions:
            ...
            if decision.reason_codes:
                try:
                    ...
                    if codes:
                        reason_codes_str = ", ".join(
                            str(c)[:8] + "..." if len(str(c)) > 8 else str(c) for c in codes[:3]
                        )
                        if len(codes) > 3:
            ...
            data.append(
                [
                    (
                        decision.created_at.strftime("%Y-%m-%d %H:%M")
                    ...
                    (
                        (decision.notes[:50] + "...")
                        if decision.notes and len(decision.notes) > 50
                        else (decision.notes or "-")
                    ),
                ]
            )
    ...
    def _build_audit_trail(self, logs: list[AuditLog]) -> list:
        ...
        for log in logs[:50]:  # Limit to 50 entries
            data.append(
                [
                    ...
                    (
                        (log.description[:60] + "...")
                        if log.description and len(log.description) > 60
                        else (log.description or "-")
                    ),
                ]
            )
        ...
        if len(logs) > 50:
            ...
            elements.append(
                Paragraph(f"<i>Showing 50 of {len(logs)} audit events</i>", self.styles["Footer"])
            )
    ...
    def _build_view_records(self, views: list[ItemView]) -> list:
        ...
        for view in views[:20]:  # Limit to 20 entries
            data.append(
                [
                    (
                        view.view_started_at.strftime("%Y-%m-%d %H:%M")
                        if view.view_started_at
                        else "N/A"
                    ),
