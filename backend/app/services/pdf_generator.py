...
class AuditPacketGenerator:
    ...
    async def _get_check_item(self, item_id: str) -> CheckItem | None:
        ...
        result = await self.db.execute(
            select(CheckItem)
            .options(selectinload(CheckItem.queue))
    ...
    def _build_header(self, item: CheckItem, generated_by: str) -> list:
        ...
        info_data = [
            ...
            ["External ID:", item.external_id or "N/A"],
        ]
    ...
    def _build_account_context(self, item: CheckItem) -> list:
        ...
        # Account info
        account_data = [
            ["Account Type:", item.account_type or "N/A", "Tenure (Days):", str(item.account_tenure_days) if item.account_tenure_days else "N/A"],
            ["Current Balance:", f"${item.current_balance:,.2f}" if item.current_balance else "N/A", "Avg Balance (30d):", f"${item.avg_balance_30d:,.2f}" if item.avg_balance_30d else "N/A"],
        ]
    ...
    def _build_images_section(self, item: CheckItem) -> list:
        ...
        if item.front_image_url:
            ...
            try:
                ...
                elements.append(Paragraph(
                    f"<i>Image Reference: {item.front_image_url}</i>",
        ...
        if item.back_image_url:
            ...
            try:
                elements.append(Paragraph(
                    f"<i>Image Reference: {item.back_image_url}</i>",
                    self.styles['FieldValue']
                ))
            except Exception:
    ...
    def _build_decision_history(self, decisions: list[Decision]) -> list:
        ...
        for decision in decisions:
            data.append([
                ...
                decision.decision_type.value.upper() if decision.decision_type else "N/A",
                decision.reviewer_id or "N/A",
    ...
    def _build_footer(self, item: CheckItem) -> list:
        ...
        Check Item ID: {item.id} | External ID: {item.external_id or 'N/A'}
