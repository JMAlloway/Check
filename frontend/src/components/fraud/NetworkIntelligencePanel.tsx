...
export default function NetworkIntelligencePanel({ checkItemId }: NetworkIntelligencePanelProps) {
  ...
  // No matches: collapse to a single slim status row instead of spending a
  // full card on an empty state (which is the common case for most items).
  if (!alertSummary?.has_alerts || activeAlerts.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 px-4 py-2.5 flex items-center gap-2 text-sm">
        <ShieldCheckIcon className="h-4 w-4 text-green-500 shrink-0" />
        <span className="font-medium text-gray-700">Network Intelligence</span>
        <span className="text-gray-500">— no cross-institution fraud matches for this check</span>
        <span
          className="ml-auto px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide bg-purple-100 text-purple-700 shrink-0"
